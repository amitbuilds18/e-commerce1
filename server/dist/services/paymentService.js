...
export const confirmPayment = async (payload) => {
    ...
    try {
        ...
        const productRows = await client.query("SELECT id, price FROM products WHERE id = ANY($1::int[])", [ids]);
        ...
        const providedTotal = payload.cart.reduce((sum, item) => sum + Number(item.price) * item.quantity, 0);
